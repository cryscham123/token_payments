...
Auth login uses a SIWE v1 message for both EOA wallets and deployed ERC-1271 smart wallet contracts. The live verifier checks deployed smart wallets with `isValidSignature(bytes32,bytes)` on the configured auth chain RPC, while browser session transport stays cookie-first through HttpOnly cookies plus CSRF double-submit. linked wallets and ERC-6492 counterfactual signature wrapping are future scope and do not add routes in this phase.
