# Step 7: stablecoin-api-store-catalog-contracts
...
Store catalog와 public API surface가 stablecoin asset과 wallet/chain compatibility를 명시적으로 다루게 한다.
...
1. `scripts/test_stablecoin_api_store_catalog_contracts.py`를 추가한다.
   ...
   - store/payment capability response는 `supportedChains`, `acceptedAssets`, `quoteId`/`expiresAt` 같은 public DTO를 사용하고, `blockchain_gas_buffer_rate` 같은 backend 설정명을 그대로 노출하지 않아야 한다.
   - API examples는 `chainName`을 payment write contract field처럼 요구하지 않아야 하며, display name이 필요하면 chain registry에서 파생된 read-only metadata로만 표현해야 한다.
2. store catalog API와 docs를 갱신한다.
   ...
   - store settlement wallet chain과 accepted asset chain의 호환성을 검증한다.
   - gas/fee 안내가 필요하면 store read DTO가 아니라 payment quote/authorization DTO에서 bounded estimate로 제공한다.
...
3. `/phases/25-multi-wallet-accounts/index.json`의 step 7 상태를 `completed`로 바꾸고 `summary`를 구체적으로 작성한다.

## 금지사항

- 상품 가격을 하나의 default native amount로만 유지하지 마라.
- disabled/unsupported token을 checkout에서 fallback 허용하지 마라.
- internal gas buffer/RPC/provider 설정명을 public API contract에 노출하지 마라.
